...
# Lightweight model metadata used only for plotting.
# Keep these lists in the same order as the paper's experiment registry.
# Defining them here avoids importing model-loading modules (torch/timm/transformers)
# when the script only needs labels and family membership.
PRH_LANGUAGE_MODELS = [
...
def _get_prh_model_labels(
    ...
    *,
    ...
    """Return plotting labels without importing model-loading modules."""
    n_lang, n_vis = shape

    if len(PRH_LANGUAGE_MODELS) == n_lang:
        llm_models = list(PRH_LANGUAGE_MODELS)
    else:
        llm_models = [f"L{i+1}" for i in range(n_lang)]

    if len(PRH_VISION_MODELS) == n_vis:
        lvm_models = list(PRH_VISION_MODELS)
    else:
...
def _extract_metric_and_k(filename: str) -> tuple[str, int | None, float | None]:
    """Extract metric name, k, and sigma from a PRH result filename.

    The parser tolerates run-configuration suffixes such as ``_n256``,
    ``_n1024``, ``_nall``, and ``_perm500`` so that the existing traversal
    logic can plot sample-size and permutation-budget experiments unchanged.
    """
    stem = Path(filename).stem
    if stem == "prh_alignment":
        return "mutual_knn", 10, None

    if stem.startswith("prh_alignment_"):
        stem = stem[len("prh_alignment_") :]

    sigma_match = re.search(r"(?:^|_)sigma([0-9]+(?:\.[0-9]+)?)(?=_|$)", stem)
    sigma_val = float(sigma_match.group(1)) if sigma_match else None

    k_match = re.search(r"(?:^|_)k(\d+)(?=_|$)", stem)
    k_val = int(k_match.group(1)) if k_match else None

    # Derive a fallback metric name from the filename. Remove only generic
    # run metadata; semantic modifiers such as dist/tau/norm are preserved.
    metric = stem
    metric = re.sub(r"(?:^|_)k\d+(?=_|$)", "", metric)
    metric = re.sub(r"(?:^|_)sigma[0-9]+(?:\.[0-9]+)?(?=_|$)", "", metric)
    metric = re.sub(r"(?:^|_)n(?:\d+|all)(?=_|$)", "", metric)
    metric = re.sub(r"(?:^|_)perm\d+(?=_|$)", "", metric)
    metric = re.sub(r"__+", "_", metric).strip("_")

    knn_metrics = ("mutual_knn", "cycle_knn", "cknna", "knn")
    if k_val is None and metric in knn_metrics:
        k_val = 10

    if metric == "cka_rbf" and sigma_val is None:
        sigma_val = 1.0
...
def _plot_prh_alignment(assets_dir: Path, *, force: bool) -> None:
    ...
    for path, payload, metric_name, k_value, sigma_value in payloads:
        ...
        # Sort language models by size using lightweight plotting metadata.
        llm_raw = list(PRH_LANGUAGE_MODELS)
        if len(llm_raw) == raw.shape[0]:
            llm_names = llm_raw
            order = list(range(len(llm_raw)))
            sizes = [_parse_llm_size_b(name) for name in llm_raw]
        ...
        # Build groups with indices into the data arrays using lightweight metadata.
        groups = []
        vision_raw = list(PRH_VISION_MODELS)
        ...
        # Build suffix directly from the input result filename.
        # This preserves topology options such as:
        #   prh_alignment_rtd_lite_tai_dist-cosine_n128_perm100.npy
        # -> _rtd_lite_tai_dist_cosine_n128_perm100
        # and prevents plots for different distance metrics from overwriting each other.
        suffix_source = path.stem
        if suffix_source == "prh_alignment":
            suffix = ""
        elif suffix_source.startswith("prh_alignment_"):
            suffix = f"_{_slug(suffix_source[len('prh_alignment_'):])}"
        else:
            suffix = f"_{_slug(suffix_source)}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot paper experiment outputs.")
    parser.add_argument(
        "--sections",
        nargs="*",
        default=["all"],
