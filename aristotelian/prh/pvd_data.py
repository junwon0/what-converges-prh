...
for the video--text extension used in this study.
...
def load_pvd_dataset(
    *,
    ...
    """Load PE-Video video bytes and captions from the Hugging Face dataset.

    The dataset is loaded in non-streaming mode (``streaming=False``). Reservoir
    sampling is then applied to valid examples encountered in dataset order.
    ...
    Args:
        ...
        auto_download: Retained for interface compatibility; data are loaded through Hugging Face Datasets.
        ...
        streaming_limit: Legacy argument name. Maximum number of valid examples
            ...
            iterates through the full split. The paper reproduction run uses 5000.
    ...
    logger.info(
        f"Loading PE-Video {split} split in non-streaming mode (sampling {max_samples} videos)"
    )

    # The reported video--text experiment loads the split in non-streaming mode.
    from datasets import Value
    ds = load_dataset("facebook/PE-Video", split=split, streaming=False)
    ds = ds.cast_column("mp4", {"bytes": Value("binary"), "path": Value("string")})
    ...
    logger.info("Iterating over loaded dataset with reservoir sampling...")
    ...
    for row in ds:
        ...
        # Early exit if the legacy streaming_limit cap has been reached
        if streaming_limit is not None and seen_count >= streaming_limit:
            logger.info(f"Reached valid-example limit of {streaming_limit} samples")
            break

    logger.info(
        f"Examined {seen_count} valid samples, selected {len(reservoir)} for reservoir"
    )

    # Sort the selected reservoir by caption for deterministic downstream ordering.
