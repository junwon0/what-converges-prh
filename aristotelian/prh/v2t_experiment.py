"""
Video-to-Text alignment experiment for the video--text extension in this study.
...
from .alignment import build_knn_log_distance_cache
from .prh_experiment import (
    ...
    _parse_knn_distance_metric,
    _prh_compute_pair_knn_distance_cached,
    _prh_compute_pair_knn_distance_values_cached,
    _prh_compute_pair_mst_distance_cached,
    _prh_compute_pair_knn_rank_cached,
    _stack_layers,
)
...
from .topology_alignment import prepare_mst_overlap_cache
...
def _extract_multi_caption_features(
    ...
    *,
    ...
    For the video--text extension, concatenates selected captions into a single string,
    ...
    """
    # Concatenate selected captions into a single string for each sample
...
def run_v2t_experiment(
    *,
    ...
    the PE-Video (PVD) dataset using the video--text protocol in this study.
    ...
    Args:
        ...
        temporal_pooling: Temporal pooling for frame-by-frame models ('single', 'mean', 'max').
            'single' uses the middle frame only (the paper setting for frame-level models).
            'mean' averages features across frames.
            'max' uses max pooling across frames.
        max_samples: Maximum number of samples to use.
        streaming_limit: Legacy argument name. Maximum number of valid examples
            to examine from the non-streaming dataset before stopping. If None,
            iterates through the full split.
    ...
    metric_for_compute, distance_tau = _parse_knn_distance_metric(metric)

    logger.info("=" * 80)
    logger.info("Video-to-Text Alignment Experiment")
    logger.info("=" * 80)
    ...
    if metric_for_compute in {"mutual_knn","knn","cycle_knn","mutual_knn_dist","mutual_knn_rank"}:
        msg = f"Metric: {metric_for_output} (base={metric_for_compute}, k={k})"
        if distance_tau is not None:
            msg += f", distance_tau={distance_tau:g}"
        logger.info(msg)
    ...
    knn_cached_metrics = {"mutual_knn","knn","cycle_knn","mutual_knn_dist","mutual_knn_rank",}
    ...
    video_grams_list = []
    text_grams_list = []
    video_log_dists_list = []
    text_log_dists_list = []
    video_mst_cache_list = []
    text_mst_cache_list = []
    ...
    if metric_for_compute in knn_cached_metrics:
        ...
        if metric_for_compute == "mutual_knn_dist":
            ...
            video_log_dists_list = [
                ...
                for layers, knn_idx in zip(video_layers_list, video_knn_list)
            ]
            text_log_dists_list = [
                build_knn_log_distance_cache(
                    layers, knn_idx,
                ...
                for layers, knn_idx in zip(text_layers_list, text_knn_list)
            ]
    elif metric_for_compute in gram_cached_metrics:
        kernel = "rbf" if metric_for_compute == "cka_rbf" else "linear"
        if metric == "cka_rbf":
            logger.info(
    ...
    if metric_for_compute == "mst_overlap_dist":
        ...
        video_mst_cache_list = [
            prepare_mst_overlap_cache(
                layers, dist_metric=topology_dist_metric, normalize=True, q=0.9,
                distance_value_geometry=distance_value_geometry,
                geodesic_k=geodesic_k,
                riemannian_k=riemannian_k,
                riemannian_dim=riemannian_dim,
            ...
            for layers in video_layers_list
        ]
        text_mst_cache_list = [
            prepare_mst_overlap_cache(
                layers, dist_metric=topology_dist_metric, normalize=True, q=0.9,
            ...
            for layers in text_layers_list
        ]
    ...
    if metric_for_compute in {"mutual_knn", "knn"}:
        ...
        configs = [
            (
                i, j,
                video_knn_list[i], text_knn_list[j],
                video_log_dists_list[i], text_log_dists_list[j],
                k, num_permutations, alpha, distance_tau, None,
            )
            for i in range(n_video)
            for j in range(n_text)
        ]
        ...
        configs = [
            (i, j, video_mst_cache_list[i], text_mst_cache_list[j],
             num_permutations, alpha, distance_tau, None)
            for i in range(n_video)
            for j in range(n_text)
        ]
    ...
    elif metric_for_compute  == "procrustes":
        configs = [
            (i, j, video_layers_list[i], text_layers_list[j], num_permutations, alpha)
            for i in range(n_video)
            for j in range(n_text)
        ...
        configs = [
            (
                ...
                video_layers_list[i],
                text_layers_list[j],
                metric_for_output if metric_for_output.startswith("mst_overlap_dist_tau") else metric_for_compute,
                k,
                num_permutations,
                alpha,
    ...
    if distance_tau is not None:
        payload["distance_tau"] = float(distance_tau)
