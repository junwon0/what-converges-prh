...
COMMON="--distance-value-geometry riemannian --riemannian-k 30 --riemannian-dim 10 --riemannian-reg 1e-3"
python -m scripts.experiments.cli --device "$DEVICE" --sections prh_alignment \
  --prh-metrics "$LOCAL" --prh-max-samples 1024 --prh-num-permutations 500 $COMMON
python -m scripts.experiments.cli --device "$DEVICE" --sections prh_alignment \
  --prh-metrics "$GLOBAL" --topology-dist-metric euclidean \
  --topology-max-samples 1024 --topology-num-permutations 500 $COMMON
